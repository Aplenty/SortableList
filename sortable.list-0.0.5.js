...
function SortableList() {
	...
	self.insertListHtml = function (template) {
        ...
        var defaultPager = "<div class='pagination-centered'>" +
            "<div data-bind='text: \" [[[Displaying]]] \" + ($root.TotalItemCount() == 0 ? 0 : ($root.CurrentPage()*$root.ItemsPerPage()-$root.ItemsPerPage()+1)) + \" - \" + ($root.CurrentPage()*$root.ItemsPerPage() > $root.TotalItemCount() ? $root.TotalItemCount() : $root.CurrentPage()*$root.ItemsPerPage()) + \" [[[of///displaying 10-20 OF 40 results]]] \" + $root.TotalItemCount() + \" [[[total results.]]]\"'>" +
                "</div><ul class='pagination'>" +
                "<li class='arrow reload-on-dom-insert' data-bind='click: $root.prevPage, css: { unavailable: $root.CurrentPage() <= 1 }'><a href=''>&laquo; [[[Prev]]]</a></li>" +
                "<!-- ko foreach: pages -->" +
                ...
                "<!-- /ko -->" +
                "<!-- /ko -->" +
                //"<li class='arrow reload-on-dom-insert' data-bind='click: $root.nextPage, css: { inactive: $root.CurrentPage() >= Math.ceil($root.TotalItemCount()/$root.ItemsPerPage()) }'><a href=''>[[[Next]]] &raquo;</a></li>" +
                "<li class='arrow reload-on-dom-insert' data-bind='click: $root.nextPage, css: { unavailable: $root.CurrentPage() >= Math.ceil($root.TotalItemCount()/$root.ItemsPerPage()) }'><a href=''>[[[Next]]] &raquo;</a></li>" +
                "</ul></div>";


        //default template
        if (typeof template == 'undefined' || template == null || template.length == 0) {
            var addButtonLabel = "[[[Add new]]]";
            if (Settings != null && Settings.hasOwnProperty("add-label"))
                addButtonLabel = Settings["add-label"];

            html +=
                //"<div class='filter'>" +
                //    "<input type='text' data-bind='value: searchText, valueUpdate: \"input\", event: {keyup: $root.search}' placeholder='[[[filter]]]' />" +
                //"</div>" +
                "<div>" +
                    "<div class='header'>" +
                        ...
                        "<div class='helper'>" +
                            "<div class='helperActions small-12 medium-2 large-3 large-push-9 columns'>" +
                             ...
                             "<div class='filter small-12 large-9 large-pull-3 columns'>" +
                                ...
                                "<input class='reload-on-dom-insert' type='text' data-bind='value: searchText, valueUpdate: \"afterkeydown\", event: {keyup: $root.search}' placeholder='[[[filter]]]' />" +
                            "</div>" +
                        "</div>"+
                    "</div>" +
								...
								"<tr class='FoldBase' data-bind='foreach: Cells, css: { \"even\": ($index() % 2 == 0) }, attr: { \"data-id\":Id }, click: function(data, event) { return $root.itemClick(data, event, $parents, true) }'>" +
									...
									"<!-- ko if: $parent.Cells().length == ($index()+1) -->" +
										...
										"<!-- ko if: $root.HideActionsColumn() == false -->" +
											"<td class='listActions FoldChildren' data-bind='attr: {\"data-heading\": \"[[[Actions]]]\"}'>" +
    ...
    self.fetchDataSet = function (done) {
        ...
        ajaxCall = $.ajax({
            ...
            .fail(function () {
            	ajaxCall = null;
            	$(Container).block({ message: '<div class="blockui-info-box">[[[Something went wrong. Try again and if the error remains please contact the system administrator.]]] <a href="#" onclick="afterFailure(' + Id + '); return false;">[[[Ok]]]</a></div>' });
            });
    };
    ...
    self.ListViewModel = function (data) {
        ...
        selfModel.getActionText = function () {
            return selfModel.isFullSize() ? "[[[Actions]]]" : "<em>[[[Click to fold down options]]]</em>";
        }
        ...
        selfModel.actionClick = function (actionData, event, parents) {
            ...
            if (typeof actionData.Url == 'function' && typeof actionData.Url() != 'undefined' && actionData.Url() != null && actionData.Url().length > 0) {
                ...
                if (actionData.ExecuteAsAjax() == true) {
                    ...
                    $(Container).block({ message: $(".loading-anim").html() });
                	//$(Container).block({ message: '<div class="blockui-info-box">[[[Loading]]]</div>' });
                    ...
                    .done(function (data) {
                    	...
                    	if ((data != null && typeof data.trim != "undefined" && data.trim() == "ok") || (data != null && data.hasOwnProperty("OperationSuccess") && data.OperationSuccess == true)) {
                    	    ...
                    	    $(Container).block({ message: '<div class="blockui-info-box">' + data.OperationMessage + ' <a href="#" onclick="afterFailure(' + Id + '); return false;">[[[Ok]]]</a></div>' });
	                    } else {
                    		$(Container).block({ message: '<div class="blockui-info-box">[[[Something went wrong. Try again and if the error remains please contact the system administrator.]]] <a href="#" onclick="afterFailure(' + Id + '); return false;">[[[Ok]]]</a></div>' });
	                    }
